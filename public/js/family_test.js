    ...
    const Family = {
        ...
        handleSelectFamily: async function (event) {
            try {
                ...
                // On va chercher la liste des familles grâce à une des routes de notre API
                ...
                const familyMembers = this.getPaginatedFamilyMembers(selectedFamily.members, 1);
                this.renderFamilyMembers(familyMembers);
                ...
                let currentPage = 1;
                const totalPages = Math.ceil(selectedFamily.members.length / this.ITEMS_PER_PAGE);
                ...
                for (let i = 1; i <= totalPages; i++) {
                    ...
                    pageButton.addEventListener('click', () => {
                        currentPage = i;
                        const familyMembers = this.getPaginatedFamilyMembers(selectedFamily.members, currentPage);
                        this.renderFamilyMembers(familyMembers);
                ...
                this.familyBlock.appendChild(pagination);
            } catch (error) {
                console.error(error);
            }
        },
        ...
        getPaginatedFamilyMembers: function(members, page) {
